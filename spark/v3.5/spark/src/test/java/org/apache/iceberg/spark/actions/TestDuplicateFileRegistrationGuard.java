 ...
 * refusal.
 *
 * <p>This branch cannot be driven end-to-end through any action, because Iceberg refuses to create
 * the state it defends against in the first place: {@code BaseRowDelta#validate} calls {@code
 * validateAddedDVs} unconditionally, so adding a second DV for a data file that already has one
 * fails at commit with "Found concurrently added DV for file..." (pinned by {@code
